...
namespace Rewired.UI.ControlMapper
{
	...
	public partial class ControlMapper : MonoBehaviour
	{
		...
		private void OnElementAssignmentConflictReplaceConfirmed(int windowId, InputMapping mapping, ElementAssignment assignment, bool skipOtherPlayers)
		{
			...
			if (skipOtherPlayers)
			{ // skip other players, only remove from system and self
		...
		private void PopulateInputFields()
		{
			...
			foreach (InputActionSet actionSet in inputGrid.GetActionSets(currentMapCategoryId))
			{
				...
				if (_showKeyboard)
				{
					...
					if (currentPlayer.controllers.hasKeyboard) PopulateInputFieldGroup(actionSet, map, controllerType, controllerId, fieldCount);
					else DisableInputFieldGroup(actionSet, ControllerType.Keyboard, fieldCount);
				}
				...
				if (_showMouse)
				{
					...
					else DisableInputFieldGroup(actionSet, ControllerType.Mouse, fieldCount);
				}
